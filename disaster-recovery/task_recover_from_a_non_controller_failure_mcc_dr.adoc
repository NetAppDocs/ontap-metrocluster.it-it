...
---
= Ripristina da un errore non del controller in una configurazione MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Verifica che i LIF siano ospitati sui nodi e sulle porte appropriate prima di eseguire uno switchback.

.Fasi
. Prima di eseguire uno switchback, verifica che i LIF siano ospitati sui nodi e sulle porte appropriate.
+
...
CAUTION: Devi verificare che la posizione delle LIF dati sui nuovi nodi sia corretta prima di eseguire lo switchback. Quando esegui lo switchback della configurazione, ONTAP tenta di riprendere il traffico sulla porta home utilizzata dalle LIF. Può verificarsi un errore di I/O se la connessione della porta home alla porta dello switch e alla VLAN non è corretta. Se riscontri problemi nel posizionare le LIF sui nodi o sulle porte corrette, non procedere con lo switchback e contatta il supporto tecnico NetApp.
...
|===
. Esegui i seguenti controlli per confermare che il passaggio di consegne tra i due cluster sia operativo:
+
.. Dal cluster di DR, controlla se è possibile eseguire un passaggio di consegne dal cluster funzionante al cluster di DR:
+
`metrocluster switchover -simulate`

.. Dal cluster rimanente, controlla se è possibile eseguire un passaggio di controllo dal cluster DR al cluster rimanente:
+
`metrocluster switchover -simulate`
...
NOTE: In un ambiente FC MetroCluster, "connections" dovrebbe risultare "not-applicable" nell'output del comando.
