...
---
= Configurare gli switch IP Cisco per l'interconnessione del cluster e la connettività IP MetroCluster back-end
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Se usi questi passaggi come parte di un'altra procedura, ad esempio la sostituzione o l'aggiornamento di uno switch, controlla se devi spostare le LIF del cluster sullo switch partner per abilitare operazioni del cluster senza interruzioni. link:task_install_and_cable_the_mcc_components.html["Scopri come configurare gli switch IP MetroCluster"].
...
.A proposito di questa attività
Questa operazione richiede un software di trasferimento file, come FTP, TFTP, SFTP, HTTP o SCP, per copiare i file sugli switch.

* Se usi questi passaggi come parte di un'altra procedura, ad esempio la sostituzione o l'aggiornamento di uno switch, controlla se devi spostare le LIF del cluster sullo switch partner per abilitare operazioni del cluster senza interruzioni. link:task_install_and_cable_the_mcc_components.html["Scopri come configurare gli switch IP MetroCluster"].
* Devi usare la versione del software dello switch supportata. Consulta la  https://hwu.netapp.com["NetApp Hardware Universe"].
...
. Ripeti i passaggi precedenti sugli altri tre switch IP nella configurazione IP MetroCluster.
+

NOTE: Devi aggiornare gli switch nel seguente ordine: Switch_A_1, Switch_B_1, Switch_A_2, Switch_B_2. Se la tua procedura richiede di spostare i cluster LIF, devi ripetere i passaggi per spostare i cluster LIF sul secondo fabric (Switch_A_2, Switch_B_2) dopo aver configurato gli switch nel primo fabric (Switch_A_1, Switch_B_1). Consulta link:task_install_and_cable_the_mcc_components.html["Scopri come configurare gli switch IP MetroCluster"].

. Verifica che il ripristino automatico sia abilitato per le LIF del cluster:
+
`network interface show -vserver Cluster -fields auto-revert`

+

CAUTION: Devi assicurarti che il ripristino automatico sia abilitato per le LIF del cluster.

. Se la funzione di ripristino automatico non è abilitata, modifica tutti i LIF del cluster per abilitarla:
+

NOTE: Se stai eseguendo questi passaggi come parte di un'altra procedura, ad esempio la sostituzione o l'aggiornamento di uno switch, potresti aver disabilitato il ripristino automatico come parte dei passaggi per link:switch-move-cluster-lifs.html["Sposta i LIF del cluster"].
