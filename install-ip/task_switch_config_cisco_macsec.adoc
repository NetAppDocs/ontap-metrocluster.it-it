...
summary: Se necessario, configura la crittografia MACsec solo sulle porte WAN ISL che collegano i siti. Se scegli di utilizzare la crittografia MACsec, configurala dopo aver applicato il file RCF corretto. 
---
= Configurare la crittografia MACsec sugli switch Cisco 9336C in un sito IP MetroCluster (facoltativo)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Se necessario, è possibile configurare la crittografia MACsec sulle porte WAN ISL che collegano i siti. Se si sceglie di utilizzare la crittografia MACsec, è necessario configurarla dopo aver applicato il file RCF corretto.

[NOTE]
====
* La crittografia MACsec può essere applicata solo alle porte WAN ISL che collegano i siti.
* La configurazione della crittografia MACsec è facoltativa durante la configurazione degli switch Cisco 9336C.
