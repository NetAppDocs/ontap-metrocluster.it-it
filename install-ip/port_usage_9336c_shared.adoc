...
---
= Assegnazione delle porte della piattaforma per switch Cisco 9336C-FX2 a 36 porte che collegano lo storage NS224 in una configurazione IP MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Se si dispone di uno switch Cisco 9336C-FX2 a 36 porte e non si prevede di collegare lo storage NS224 allo switch, utilizzare le tabelle in link:port_usage_3232c_9336c.html["Assegnazioni delle porte della piattaforma per gli switch Cisco 3232C o Cisco 9336C-FX2 a 36 porte"].
...
* Quando si collega uno switch Cisco 9336C-FX2 che connette lo storage NS224, è possibile avere un massimo di due configurazioni o gruppi DR MetroCluster. Almeno una configurazione o un gruppo DR MetroCluster deve collegare shelf NS224 allo switch MetroCluster.
+
Se una delle tue configurazioni MetroCluster o dei gruppi DR non collega gli shelf NS224 allo switch MetroCluster, segui la <<tables_not_connecting_ns224,tabelle di cablaggio per controllori che non collegano ripiani NS224 collegati all'interruttore>>.

* RcfFileGenerator mostra solo le piattaforme idonee quando viene selezionata la prima piattaforma.
* Se si sta espandendo una configurazione MetroCluster standard che non supporta l'archiviazione NS224 collegata allo switch e si desidera aggiungere una configurazione MetroCluster dotata di archiviazione NS224 collegata allo switch, esaminare le seguenti informazioni:
+
** Nello link:https://mysupport.netapp.com/site/tools/tool-eula/rcffilegenerator["RcfFileGenerator"^] strumento, è necessario selezionare prima la piattaforma che supporta l'archiviazione NS224 collegata allo switch, quindi selezionare la piattaforma che non supporta l'archiviazione NS224 collegata allo switch.
+
Ciò significa che quando si genera l'RCF per la nuova configurazione, potrebbe essere necessario selezionare le piattaforme nello strumento RcfFileGenerator nel seguente ordine:

+
... Seleziona la piattaforma che supporta lo storage NS224 collegato allo switch dal *secondo campo a discesa*.
... Selezionare la piattaforma che non supporta lo storage NS224 collegato allo switch dal *primo campo a discesa*.
