...
---
= Requisiti IP MetroCluster per l'assegnazione automatica delle unità e per i sistemi ADP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Quando si utilizza ADP con configurazioni IP MetroCluster , è necessario tenere presente quanto segue:
...
Prima di utilizzare le tabelle, verificare i seguenti requisiti:

* È necessario rispettare i limiti MetroCluster per il numero massimo di unità supportate e altre linee guida. Fare riferimento al https://hwu.netapp.com["NetApp Hardware Universe"] .
* Se si riutilizza uno scaffale per dischi esterno, verificare che la proprietà del disco sullo scaffale per dischi esterno sia stata rimossa prima di collegarlo al controller. Fare riferimento alink:https://docs.netapp.com/us-en/ontap/disks-aggregates/remove-ownership-disk-task.html["Rimuovere la proprietà ONTAP da un disco"^] .
...
=== Assegnazione del disco sui sistemi FAS50 con Flash Cache

A partire da ONTAP 9.18.1, Flash Cache è supportato sui sistemi FAS50 per le configurazioni IP MetroCluster .

[CAUTION]
====
* Non è possibile avere sia aggregati di dati sia l'aggregato radice con unità Flash Cache sullo shelf interno.
* Gli slot 0 e 23 sono utilizzati per le unità Flash Cache.
* Se si riutilizza uno scaffale per dischi esterno, verificare che la proprietà del disco sullo scaffale per dischi esterno sia stata rimossa prima di collegarlo al controller. Fare riferimento alink:https://docs.netapp.com/us-en/ontap/disks-aggregates/remove-ownership-disk-task.html["Rimuovere la proprietà ONTAP da un disco"] .
...
Non applicabile
 ...
 a| 
36 unità (12 unità interne e 24 unità esterne, con gli aggregati di dati sullo scaffale esterno e l'aggregato radice sullo scaffale interno)
 a| 
Le unità interne sono divise in quattro gruppi uguali (quarti).  Ogni trimestre viene assegnato automaticamente a un pool separato.  Le unità sui ripiani esterni sono divise in quattro gruppi uguali (quarti).  Ogni quarto di scaffale viene assegnato automaticamente a un pool separato.

*Note:*

* Se si utilizzano unità interne ed esterne, è necessario prima installare ONTAP e configurare il cluster locale solo con le unità interne.  Dopo aver completato l'installazione o la configurazione, è possibile collegare manualmente lo scaffale esterno.
* Per l'aggregato radice sono necessarie almeno 12 unità interne.  Dovresti posizionare le unità interne principali dallo slot 1.  Ad esempio, per 12 unità interne, utilizzare gli slot 1-3, 6-8, 12-14 e 18-20.
