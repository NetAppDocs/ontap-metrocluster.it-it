...
NOTE: È possibile configurare un ONTAP Mediator che utilizza HTTPS solo tramite CLI. System Manager non è supportato. Per ulteriori informazioni, consultare link:concept_mediator_requirements.html#select-a-communication-protocol-for-ontap-mediator["Selezionare un protocollo di comunicazione per ONTAP Mediator"].
