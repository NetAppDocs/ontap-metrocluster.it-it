...
---
= Requisiti per gli ISL IP di MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
== Requisiti MetroCluster ISL

Questi requisiti si applicano agli ISL su tutte le configurazioni IP MetroCluster:
...
* La velocità ISL minima richiesta dipende dal modello della piattaforma:
+
** A partire da ONTAP 9.18.1, le piattaforme con una velocità della porta backend IP MetroCluster di 100Gbps richiedono una velocità minima del collegamento ISL di 100Gbps. L'utilizzo di una velocità ISL diversa richiede una Feature Policy Variance Request (FPVR). Per presentare una FPVR, contattare il team di vendita NetApp.
+

CAUTION: Gli aggiornamenti a ONTAP 9.18.1 e versioni successive su piattaforme che attualmente non soddisfano il requisito ISL a 100Gbps non sono bloccati e possono procedere. Tuttavia, NetApp raccomanda vivamente ai clienti di passare alla connettività ISL a 100Gbps per mantenere i livelli di prestazioni e disponibilità previsti.

** Su tutte le altre piattaforme, la velocità minima di collegamento ISL supportata è 10 Gbps.


* Deve essere presente almeno una porta ISL da 10Gbps per fabric.
