...
[role="lead"]
Identificare i componenti hardware e software richiesti e supportati per una configurazione IP MetroCluster .  Esaminare le convenzioni di denominazione utilizzate negli esempi di documentazione per i componenti.
...
*Nota:* La velocità di 25 GbE richiede l'adattatore ricetrasmettitore MAM1Q00A-QSA28.
...
NOTE: I nomi degli aggregati devono essere univoci in tutti i siti MetroCluster. Ciò significa che non è possibile avere due aggregati diversi con lo stesso nome sul sito A e sul sito B.
