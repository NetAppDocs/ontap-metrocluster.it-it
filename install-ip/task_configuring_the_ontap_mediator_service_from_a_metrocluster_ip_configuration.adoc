...
* Questa operazione può essere eseguita sull'interfaccia ONTAP di qualsiasi nodo nella configurazione IP MetroCluster.
* A partire da ONTAP 9.18.1 e ONTAP Mediator 1.11, una singola istanza di ONTAP Mediator può gestire fino a dieci MetroCluster configurazioni contemporaneamente. Nelle versioni precedenti, ONTAP Mediator può supportare fino a cinque MetroCluster configurazioni contemporaneamente.
...
.Utilizzare IPv4
--
.. Eseguire il seguente comando per aggiungere l'ONTAP Mediator:
+
`metrocluster configuration-settings mediator add -mediator-address <mediator_host_ip_address>`

+

NOTE: Ti verrà richiesto di inserire il nome utente e la password per l'account admin utente di Mediator.
...
. Facoltativamente, configura il passaggio forzato automatico di MetroCluster.
+
È possibile utilizzare solo il seguente comando a livello di privilegi avanzati.

+

IMPORTANT: Prima di utilizzare questo comando, rivedere il link:concept-ontap-mediator-supports-automatic-unplanned-switchover.html#mauso-9-12-1["Rischi e limitazioni dell'utilizzo dello switchover forzato automatico di MetroCluster"].
