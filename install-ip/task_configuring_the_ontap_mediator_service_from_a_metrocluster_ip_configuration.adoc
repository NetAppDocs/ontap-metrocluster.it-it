...
* Il supporto per il monitoraggio di più configurazioni IP di MetroCluster utilizzando un'unica istanza di ONTAP Mediator dipende dal fatto che tu configuri ONTAP Mediator con HTTPS o iSCSI. Per ulteriori informazioni, consulta link:concept_mediator_requirements.html#mediator-multiple-mccip["Utilizza ONTAP Mediator per monitorare più configurazioni IP MetroCluster"].
