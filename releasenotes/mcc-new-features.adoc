...
Nuove combinazioni di aggiornamento supportate per MetroCluster aggiornamenti del controller IP utilizzando  `system controller replace` comandi
 a| 
Supporto per gli aggiornamenti di sistema da AFF A70 ad AFF A90 e da FAS70 a FAS90 utilizzando i comandi  `system controller replace` in una configurazione IP MetroCluster.
