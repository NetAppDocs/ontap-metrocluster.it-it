...
---
= Sostituisci un singolo bridge FC-to-SAS in una configurazione FC di MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Collegare i cavi FC alle stesse porte del nuovo bridge.
. Aggiornare il firmware FibreBridge su ciascun bridge.
+
Se il nuovo bridge è dello stesso tipo del bridge partner, eseguire l'aggiornamento allo stesso firmware del bridge partner. Se il nuovo bridge è di tipo diverso da quello del bridge partner, eseguire l'aggiornamento al firmware più recente supportato dal bridge e dalla versione di ONTAP. Vedere link:task_update_firmware_on_a_fibrebridge_bridge_parent_topic.html["Aggiornamento del firmware su un bridge FibreBridge"]

. [[step18-reconnect-newbridge]] ricollegare i cavi SAS alle stesse porte del nuovo bridge.
+
...
. [[step19-verify-each-bridge]]verificare che ciascun bridge sia in grado di visualizzare tutti i dischi e gli shelf di dischi a cui è collegato il bridge.
+
[cols="35,65"]
|===
...
Esatto
 a| 
Ripetere l'operazione <<step19-verify-each-bridge,Passaggio 19>> per ciascun ponte rimanente.
...
Non corretto
 a| 
.. Verificare l'eventuale presenza di cavi SAS allentati o correggere il cablaggio SAS ripetendo la procedura <<step18-reconnect-newbridge,Fase 18>>.
.. Ripetere <<step19-verify-each-bridge,Passaggio 19>>.
...
. Aggiornare il firmware FibreBridge su ciascun bridge.
+
link:task_update_firmware_on_a_fibrebridge_bridge_parent_topic.html["Aggiornare il firmware su un bridge FibreBridge"]

. [[step18-reconnect-cables]]ricollegare i cavi SAS alle stesse porte del nuovo bridge.
+
...
Non corretto
 a| 
.. Verificare l'eventuale presenza di cavi SAS allentati o correggere il cablaggio SAS ripetendo la procedura <<step18-reconnect-cables,Fase 18>>.
...
. Collegare i cavi FC alle porte Fibre Channel 1 sul nuovo bridge.
+
È necessario collegare la porta FC allo stesso switch o alla stessa porta del controller a cui era stato collegato il ponte FibreBridge 6500N.

. Aggiornare il firmware FibreBridge su ciascun bridge.
+
Se il nuovo bridge è dello stesso tipo del bridge partner, eseguire l'aggiornamento allo stesso firmware del bridge partner. Se il nuovo bridge è di tipo diverso dal bridge partner, eseguire l'aggiornamento al firmware e alla versione più recenti di ONTAP supportati dal bridge.

+
link:task_update_firmware_on_a_fibrebridge_bridge_parent_topic.html["Aggiornare il firmware su un bridge FibreBridge"]

. [[step18-Recable]]ricollegare i cavi SAS alle porte SAS A sul nuovo bridge.
+
...
Non corretto
 a| 
.. Verificare l'eventuale presenza di cavi SAS allentati o correggere il cablaggio SAS ripetendo la procedura <<step18-recable,Fase 18>>.
.. Ripetere il passaggio precedente per ogni bridge rimanente.
...
. Dopo aver sostituito il componente, restituisci il componente difettoso a NetApp, come descritto nelle istruzioni RMA fornite con il kit. Vedi la pagina link:https://mysupport.netapp.com/site/info/rma["Amp per restituzione parti; sostituzioni"^] per ulteriori informazioni.
