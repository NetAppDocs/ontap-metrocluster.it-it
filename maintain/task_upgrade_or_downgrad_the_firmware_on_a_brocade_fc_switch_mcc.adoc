...
---
= Aggiorna o esegui il downgrade del firmware su uno switch Brocade FC in una configurazione FC MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Prima di iniziare
Leggi attentamente le seguenti informazioni prima di utilizzare questa procedura:

* A partire da Fabric OS 10.x, Brocade supporta solo SHA-512 per l'autenticazione SNMP. ONTAP attualmente non supporta SHA-512, con il supporto limitato a SHA-256.
+
Se aggiorni il firmware dello switch FC MetroCluster a Fabric OS 10.x, la visibilità del monitoraggio SNMP da ONTAP andrà persa. Questo comporta un rischio maggiore durante la risoluzione dei problemi e la perdita degli avvisi proattivi.

+
Se esegui l'aggiornamento o il downgrade da Fabric OS 10.x, devi svolgere le attività obbligatorie descritte in link:https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-software-upgrade/10-0-x/Upgrading-and-Downgrading-Firmware/v24512876.html["Brocade Fabric OS Software Upgrade Guide, 10.0.x - Prerequisiti per l'aggiornamento o il downgrade"^].

+

CAUTION: Devi valutare attentamente queste limitazioni prima di procedere con l'aggiornamento o il downgrade a o da Fabric OS 10.x o versioni successive.

* A partire da Fabric OS 9.0.1, SNMPv2 non è supportato sugli switch Brocade. Se aggiorni a Fabric OS 9.0.1 o versioni successive, devi usare SNMPv3 per il monitoraggio dello stato di salute. Per ulteriori informazioni, vedi link:../install-fc/concept_configure_the_mcc_software_in_ontap.html#snmpv3-mcc-fc["Configurazione di SNMPv3 in una configurazione MetroCluster"].
* Se stai aggiornando a Fabric OS 9.2.x o versioni successive, devi avere un certificato Brocade TruFOS installato. Consulta link:https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-software-upgrade/9-2-x/Obtaining-Firmware/download-prerequisites-new/brocade-trufos-certificates.html["Guida all'aggiornamento del software del sistema operativo Brocade Fabric, 9,2.x"^] per maggiori informazioni.


.A proposito di questa attività
Verificare di aver controllato ed eseguito le seguenti attività per la configurazione:

* Verifica che la nuova versione del firmware sia compatibile con la tua soluzione. Consulta il link:https://hwu.netapp.com/["Hardware Universe"^] per maggiori informazioni.
