...
---
= Sostituisci o riutilizza uno switch in una configurazione IP MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Ripristina tutti i LIF del cluster alla loro porta home.
+

CAUTION: Non ripristinare le LIF del cluster nelle rispettive porte home finché non hai completato i passaggi precedenti per tutti gli switch che richiedono un aggiornamento.
...
----
.. Ripristina manualmente tutte le LIF del cluster che al momento non si trovano sulle rispettive porte home alle loro porte home:
+
