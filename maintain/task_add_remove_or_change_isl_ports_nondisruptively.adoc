...
---
= Aggiungi, rimuovi o modifica le porte ISL senza interruzioni sugli switch Cisco in una configurazione IP MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Se usi uno switch Cisco 9336C-FX2 in una configurazione condivisa che collega shelf NS224, cambiare le ISL potrebbe richiedere un nuovo file di configurazione (RCF). Non ti serve un nuovo RCF se le velocità ISL attuale e nuova sono 40 Gbps e 100 Gbps. Tutte le altre modifiche alla velocità ISL richiedono un nuovo RCF. Ad esempio, cambiare la velocità ISL da 40 Gbps a 100 Gbps non richiede un nuovo RCF, ma cambiare la velocità ISL da 10 Gbps a 40 Gbps richiede un nuovo RCF.
...
NOTE: Se usi gli switch Cisco 9336C-FX2 in una configurazione condivisa che collega gli shelf NS224 e devi aggiornare l'RCF e applicare la nuova configurazione per le nuove porte ISL, segui i passaggi per link:task_upgrade_rcf_files_on_mcc_ip_switches.html["aggiorna gli RCF sugli switch IP MetroCluster."]
