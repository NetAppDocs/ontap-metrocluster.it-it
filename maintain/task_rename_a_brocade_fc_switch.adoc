...
summary: Potrebbe essere necessario rinominare uno switch FC Brocade per garantire un nome coerente in tutta la configurazione. 
---
= Rinomina uno switch Brocade FC in una configurazione FC MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Fasi
. Disabilitare in modo persistente lo switch o gli switch in un fabric:
+
...
7840_FCIP_2:admin>
----
. Rinominare lo switch o gli switch:
+
...
+
L'esempio seguente mostra l'output per `*switchname _new-switch-name_*` comando:
...
----
. Abilitare costantemente gli switch: `*switchcfgpersistentenable*`
+
