...
---
= Ricollega e riconfigura gli switch FC MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Devi applicare i file di configurazione di riferimento (RCF) per riconfigurare la suddivisione in zone e includere i nuovi nodi.

Se non puoi usare gli RCF per configurare gli switch, devi configurarli manualmente. Vedi:
...
.Fasi
. Individua i RCF per la tua configurazione.
+
Devi usare gli RCF per una configurazione a otto nodi che corrisponde al tuo modello di switch.

. Applica i file RCF seguendo le istruzioni nella pagina di download, regolando le impostazioni ISL secondo necessità.
...
+
L'utilizzo della porta dello switch FC deve corrispondere all'utilizzo a otto nodi di MetroCluster descritto in link:../install-fc/index.html["Installazione e configurazione di Fabric-Attached MetroCluster"] modo che tu possa usare gli RCF.
