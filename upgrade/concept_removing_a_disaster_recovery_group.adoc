...
NOTE: Questi passaggi vengono utilizzati durante i flussi di lavoro di transizione e aggiornamento del sistema. È possibile che alcuni passaggi di questa procedura siano già stati completati nell'ambito di tali flussi di lavoro.
...
NOTE: Il `metrocluster remove-dr-group` Il comando è supportato solo su ONTAP 9.8 e versioni successive.
...
+
In una configurazione MetroCluster FC, vai al passaggio per <<fc_step_7_remove_nodes,rimozione dei vecchi nodi del gruppo DR>>.
...
.. Rimuovere la proprietà dei dischi del pool1 su ciascun nodo del gruppo DR da eliminare. Eseguire questi passaggi su ciascun nodo da rimuovere. Negli esempi utilizzati in questo passaggio, viene rimosso il nodo node_A_1.
+
... Vai al nodeshell:
+
`run -node <node_name>`
...
+
Vengono visualizzati tutti i dischi, inclusi i dischi Pool1 appartenenti al nodo. L'esempio seguente mostra un output troncato in cui il disco 0n.4 e le sue partizioni sono assegnati a Pool1. L'output varierà a seconda della configurazione.
...
... Rimuovere la proprietà del disco per ciascun disco del pool1:
+
Se la configurazione contiene dischi partizionati, è necessario prima rimuovere la proprietà di ciascun disco partizionato e poi rimuovere la proprietà del disco contenitore:

+
`disk remove_ownership <disk_name>`

+
Nell'esempio seguente, la proprietà dei dischi partizionati viene rimossa prima che la proprietà del disco contenitore `0n.4` venga rimossa:
...
. [[fc_step_7_remove_nodes]]Rimuovi i nodi nel vecchio gruppo DR.
+
...
Per saperne di più sui comandi utilizzati in questa procedura, consultare la link:https://docs.netapp.com/us-en/ontap-cli/["Riferimento ai comandi ONTAP"^].
