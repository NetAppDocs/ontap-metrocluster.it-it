...
+
Utilizza i passaggi in link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["Impostare le variabili di boot IP di MetroCluster"].
...
NOTE: Quando esegui l'opzione del menu di boot  `6`, potrebbe essere necessario reimpostare i valori di bootarg di MetroCluster. Consulta link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["Impostare le variabili di boot IP di MetroCluster"].
...
.. Dopo che i LIF intercluster sono attivi, controllare lo stato dei peer del cluster e ristabilire il peering del cluster secondo necessità.
+
