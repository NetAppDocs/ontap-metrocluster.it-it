...
---
= Flusso di lavoro di aggiornamento del controller IP di MetroCluster tramite sostituzione del controller di sistema
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
A partire da ONTAP 9.13.1, puoi utilizzare questa procedura guidata di switchover automatizzato di MetroCluster per eseguire un aggiornamento del controller senza interruzioni su una configurazione IP MetroCluster a quattro nodi utilizzando i comandi `system controller replace`. Altri componenti (come shelf di storage o switch) non possono essere aggiornati nell'ambito di questa procedura.
