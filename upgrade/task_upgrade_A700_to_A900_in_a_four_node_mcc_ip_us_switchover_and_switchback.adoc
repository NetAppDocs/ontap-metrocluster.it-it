...
---
= Aggiorna AFF A700 ad AFF A900 o FAS9000 a FAS9500 in una configurazione IP MetroCluster usando switchover e switchback
:allow-uri-read: 


[role="lead"]
A partire da ONTAP 9.10.1, puoi utilizzare l'operazione di switchover di MetroCluster per fornire un servizio senza interruzioni ai client durante l'aggiornamento dei moduli controller sul cluster partner. Altri componenti (come shelf di storage o switch) non possono essere aggiornati nell'ambito di questa procedura.
...
image::../media/workflow_ip_upgrade.png[Flusso di lavoro per l'aggiornamento dei controller nella configurazione IP MetroCluster]
...
** link:../install-ip/task_switch_config_cisco.html#reset-the-cisco-ip-switch-to-factory-defaults["Ripristinare lo switch IP Cisco alle impostazioni predefinite"]
...
** link:../install-ip/task_switch_config_cisco.html#download-and-install-the-cisco-ip-rcf-files["Scaricare e installare i file RCF IP di Cisco"]
...
. Se i nodi MetroCluster sono configurati con un gestore di chiavi esterno per l'SVM di amministrazione (cluster), copiare le informazioni del server delle chiavi in una posizione sicura. Se è configurato l'Onboard Key Manager, copiare le informazioni di backup e la passphrase in una posizione sicura.
+
[role="tabbed-block"]
====
.Onboard Key Manager
--
.. Eseguire il seguente comando per visualizzare il backup di Onboard Key Manager:
+
`security key-manager onboard show-backup`

+
Durante la procedura di aggiornamento, avrai bisogno anche della passphrase più aggiornata di Onboard Key Manager.
...
--
Raccogli le informazioni sulla gestione delle chiavi esterne che ti serviranno per ripristinare la configurazione del key manager in una fase successiva della procedura.

.. Visualizza il nome dell'SVM amministratore (cluster).
+
`vserver show -type admin`

+
In questo esempio, il nome dell'SVM amministratore è cluster_B:
...
cluster_B
            admin   -          -          -           -          -
----
.. Ottenere l'indirizzo e il numero di porta di un server di chiave primaria associato al gestore di chiavi esterno configurato sull'SVM di amministrazione (cluster):
+
[NOTE]
====
*** L'SVM di amministrazione può avere fino a quattro server di chiavi primarie, ma avrai bisogno solo del numero di porta e dell'indirizzo di uno dei server di chiavi primarie per ripristinare la configurazione di gestione delle chiavi più avanti nella procedura.
*** Se al server della chiave primaria che hai scelto sono associati un nome utente e una password, avrai bisogno anche di queste informazioni per la procedura di ripristino.
...
----
.. Se si utilizza ONTAP 9.16.1 o versioni successive, ottenere l'UUID del keystore per l'SVM di amministrazione.
+
...
cluster_B::*
----
.. Salva le informazioni raccolte in un luogo sicuro per utilizzarle in seguito durante la procedura. Dovresti aver raccolto le seguenti informazioni:
+
*** Il numero di porta e l'indirizzo di uno dei server chiave primari per l'SVM di amministrazione
*** Il nome utente e la password se associati al server della chiave primaria scelto
*** L'UUID del keystore per l'SVM di amministrazione (richiesto in ONTAP 9.16.1 o versioni successive)
...
Spareggio
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#remove-metrocluster-configurations["Rimozione delle configurazioni MetroCluster"] nel contenuto _Installazione e configurazione di MetroCluster Tiebreaker_
...
image::../media/mcc_upgrade_cluster_a_in_switchover_A900.png[Site_B inattivo e pronto per l'aggiornamento dopo il passaggio a MetroCluster]
...
+
image::../media/drw_9500_remove_PCM.png[Rimozione del modulo controller dallo chassis: sbloccare e far scorrere verso l'esterno]
...
+
image::../media/drw_a900_move-remove_NVRAM_module.png[Rimozione del modulo NVRAM dallo chassis: sbloccare e tirare le linguette]
...
+
image::../media/drw_a900_move-remove_NVRAM_module.png[Installazione del modulo NVRAM nello chassis: allineare e bloccare in posizione]
...
+
image::../media/drw_9500_remove_PCM.png[Installazione del modulo controller nel telaio: allineamento, posizionamento e cablaggio]
...
.Prima di iniziare
* Verifica di poter accedere a un server HTTP con il sistema.
...
+
.. Entra `n` per saltare il ripristino del backup quando viene visualizzato il seguente messaggio:
+
....
Do you want to restore the backup configuration now? {y|n} n
....
.. Entra `y` per riavviare quando viene visualizzato il seguente messaggio:
+
....
...
NOTE: Per poter utilizzare il software appena installato è necessario riavviare il nodo.
...
.Fasi
. Prepara gli switch IP per l'applicazione dei nuovi file di configurazione di riferimento (RCF).
+
Seguire la procedura descritta nella sezione relativa al fornitore dello switch:
...
** link:../install-ip/task_switch_config_cisco.html#download-and-installing-the-cisco-ip-rcf-files["Scaricare e installare gli RCF IP Cisco"]
...
. Se l'SVM di amministrazione del MetroCluster system è configurato con un key manager, selezionare l'opzione corretta del menu di boot per la configurazione della gestione delle chiavi.
+
|===
...
Segui le istruzioni e fornisci i seguenti dati per recuperare e ripristinare la configurazione del key manager:

** Backup e passphrase di Onboard Key Manager
...
*Nota:* Sebbene l'SVM amministratore possa avere fino a quattro server di chiave primaria, durante il processo di ripristino è possibile specificare le informazioni relative a un solo server di chiave primaria.
...
. Se si utilizza la crittografia del volume root, selezionare nuovamente l'opzione del menu di boot per la configurazione della gestione delle chiavi.
+
|===
...
NOTE: Quando esegui l'opzione del menu di boot `6`, i valori di bootarg di MetroCluster potrebbero dover essere reimpostati. Consulta link:task_upgrade_A700_to_A900_in_a_four_node_mcc_ip_us_switchover_and_switchback.html#set-the-metrocluster-ip-bootarg-variables["Impostare le variabili di boot IP di MetroCluster"].
...
image::../media/mcc_upgrade_cluster_a_switchback_A900.png[Site_A in attesa di aggiornamento dopo il passaggio a MetroCluster]
