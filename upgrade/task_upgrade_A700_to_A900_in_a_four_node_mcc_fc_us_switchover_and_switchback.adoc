...
---
= Aggiorna AFF A700 ad AFF A900 o FAS9000 a FAS9500 in una configurazione FC MetroCluster utilizzando switchover e switchback
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
A partire da ONTAP 9.10.1, puoi utilizzare l'operazione di switchover di MetroCluster per fornire un servizio senza interruzioni ai client mentre aggiorni i moduli controller sul cluster partner. Non puoi aggiornare altri componenti (come shelf di storage o switch) nell'ambito di questa procedura.
...
+
Lo stato HA per tutti i componenti dovrebbe essere  `mcc`.
...
. Su ciascun nodo, controlla la data, l'ora e il fuso orario del sistema:
+
`show date`

. Se necessario, imposta la data in UTC o Greenwich Mean Time (GMT):
+
`set date <mm/dd/yyyy>`

. Controlla l'ora utilizzando il seguente comando al prompt dell'ambiente di avvio:
+
`show time`

. Se necessario, imposta l'ora in UTC o GMT:
+
`set time <hh:mm:ss>`
...
. Raccogli le variabili d'ambiente:
+
`printenv`
...
|===
. Verifica la modalità ha-config:
+
`ha-config show`
...
. Utilizzare i passaggi indicati in: link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Aggiunta di configurazioni MetroCluster"] nella sezione _Installazione e configurazione di MetroCluster Tiebreaker_.
