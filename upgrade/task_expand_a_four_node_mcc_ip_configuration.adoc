...
* La tabella mostra tutti i modelli di piattaforma che utilizzano *porte MetroCluster/HA condivise* e *porte cluster/HA condivise*. Non tutte le combinazioni sono supportate da questa procedura. Consulta la <<expand-ip-supported-platforms,combinazioni di piattaforme supportate>> per verificare se la tua combinazione di espansione è supportata.
...
Le tabelle seguenti mostrano le combinazioni di piattaforme supportate per configurazioni IP a otto nodi MetroCluster.
...
image:../media/expand-ip-group-1-updated.png["Combinazioni di espansione per piattaforme AFF e FAS gruppo 1 in MetroCluster IP"]
...
image:../media/expand-ip-group-2-updated.png["Combinazioni di espansione per piattaforme AFF e FAS gruppo 2 in MetroCluster IP"]
...
== Rimuovere la configurazione dalle applicazioni di monitoraggio
...
== Preparare i nuovi moduli del controller
...
. Dalla modalità Maintenance, immettere il comando halt per uscire dalla modalità Maintenance:
+
`halt`

. Verificare se il partner-sysid è corretto:
+
...
. Avvia il sistema e accedi alla configurazione del cluster:
+
`boot_ontap`
...
== Configurare le LIF intercluster per il peering tra cluster
...
+
Per la sintassi completa del comando, vedere link:https://docs.netapp.com/us-en/ontap-cli["Riferimento ai comandi ONTAP"^].
...
+
L'esempio seguente assegna le porte "e0e" e "e0f" al gruppo di failover "intercluster01" sull'SVM "cluster01":
...
+
L'esempio seguente mostra che le LIF intercluster "cluster01_icl01" e "cluster01_icl02" sulla porta "e0e" effettueranno il failover sulla porta "e0f".
...
+
Dovresti visualizzare un output simile all'esempio seguente:
...
== Creare le interfacce MetroCluster e gli aggregati root mirror

È necessario creare le interfacce MetroCluster sui nuovi nodi IP MetroCluster e mirroring degli aggregati root.
...
.Fasi
. Su ciascun sito, aggiungere i nuovi LIF intercluster alla relazione peer.
+
Dal cluster_A, eseguire il seguente comando:
...
+
Dal cluster_B, eseguire il seguente comando:
...
NOTE: L'output del  `metrocluster configuration-settings connection show` command può variare a seconda del modello della piattaforma. Ad esempio, su alcune piattaforme, le righe per HA Partner non vengono visualizzate nell'output.
...
== Completare l'aggiunta dei nuovi nodi
...
.. Aggiorna la configurazione MetroCluster su uno qualsiasi dei nodi appena aggiunti:
+
[cols="30,70"]
|===
