...
---
= Aggiorna i controller utilizzando la sostituzione del controller di sistema in una configurazione FC MetroCluster a quattro nodi
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
A partire da ONTAP 9.10.1, puoi utilizzare questa procedura guidata di switchover automatizzato di MetroCluster per eseguire un aggiornamento del controller senza interruzioni su una configurazione FC MetroCluster a quattro nodi utilizzando i comandi `system controller replace`. Non puoi aggiornare altri componenti (come shelf di storage o switch) nell'ambito di questa procedura.
...
* Puoi usare questa procedura per aggiornare i controller in una configurazione FC a quattro nodi MetroCluster utilizzando il passaggio automatico e il ripristino automatici basati su NSO. Se vuoi eseguire un aggiornamento del controller utilizzando la riallocazione aggregata (ARL), consulta link:https://docs.netapp.com/us-en/ontap-systems-upgrade/upgrade-arl-auto-app/["Utilizzare i comandi "System controller replace" per aggiornare l'hardware del controller con ONTAP 9.8 o versione successiva"^]. Si consiglia di usare la procedura automatizzata basata su NSO.
...
....
. Entra `n` per saltare il ripristino del backup quando viene visualizzato un messaggio simile al seguente:
+
....
